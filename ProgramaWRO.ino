...
while(!listo2){
    ...
    if(DERECHA>=22 && IZQUIERDA>=22 && ENFRENTE>=DLim) //Cuidar que no estemos muy pegados 
    {
    ReceiveSerialFull();
    ColorAnt=Color;
    if(Sector>0)
    ...
    else if(ENFRENTE<=DLim && IZQUIERDA>=100)
    {
    ...
    SetServo();
      ...
      else if(ENFRENTE<=DLim && DERECHA>=100)
    {
...
//Si se acaba la memoria, tal vez hacer los casos e irlos llamando porn turnos en el primer while, luego quedarse en uno solo
    ...
    if(DERECHA>=22 && IZQUIERDA>=22 && ENFRENTE>=DLim)
    {
    ReceiveSerialFull();
    ColorAnt=Color;
    if(Sector>0)
